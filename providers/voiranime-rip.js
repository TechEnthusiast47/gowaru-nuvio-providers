 ...
 * Generated: 2026-06-03T20:38:27.046106943Z
 */
var __provider = (() => {
  ...
  var HEADERS, _atob, CODEC_PREFERENCE, STRICT_QUALITY_TIERS, DEFAULT_QUALITY_TIER, CODEC_PRIORITY, manifestCache, MANIFEST_CACHE_TTL, BASE_URL_FORBIDDEN_PATTERN;
  ...
  var SITE, ENDPOINTS, PATTERNS, TIMEOUTS, SCORES, CACHE_TTL, MAX_SEARCH_TITLES;
        ...
        try {
          ...
          if (!res || !res.ok) {
            ...
            if (attempt < retries && status >= 500) {
              ...
              yield sleep(RETRY_DELAYS[attempt] || 2e3);
              continue;
            }
  ...
  var HEADERS2, AJAX_HEADERS, RETRY_DELAYS;
  var init_http = __esm({
    "src/voiranime-rip/http.js"() {
      init_resolvers();
      init_config();
  ...
  function searchAnime(titles) {
    return __async(this, null, function* () {
      for (const title of titles.slice(0, MAX_SEARCH_TITLES)) {
        try {
          const html = yield postSearch(title, { timeout: TIMEOUTS.SEARCH });
          const results = parseSearchResults(html);
          if (results.length === 0) continue;
          const scored = results.map((r) => __spreadProps(__spreadValues({}, r), { score: scoreMatch(r.title, title) })).filter((r) => r.score >= SCORES.MIN_MATCH).sort((a, b) => b.score - a.score);
  ...
  function extractStreams(tmdbId, mediaType, season, episode) {
    return __async(this, null, function* () {
      ...
      const subType = yield detectSubType(tmdbId, mediaType);
  ...
  var require_index = __commonJS({
    "src/voiranime-rip/index.js"(exports, module) {
      init_extractor();
      init_resolvers();
      var PROVIDER_TIMEOUT = safeConfig("NUVIO_TIMEOUT_VOIRANIMERIP", 6e4);
